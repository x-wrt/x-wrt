...
proto_qmi_setup() {
	...
	while true; do
		serving_system="$(uqmi -s -d "$device" -t 3000 --get-serving-system 2>/dev/null)"
	...
	uqmi -s -d "$device" -t 3000 --modify-profile "3gpp,$profile_id" --apn "$apn" --pdp-type "$profile_pdptype" > /dev/null 2>&1
